...
// Produce recognition uses the Claude-Vision endpoint (recognize-llm) rather
// than the Baidu one (recognize): same request/response shape, but far more
// robust on hand-held / plastic / poorly-framed shots where Baidu returns
// Uncertain. Request/response contract is identical, so the parsing below is
// unchanged (raw_name is always null on this endpoint).
static const char* PRODUCE_URL = "https://snapchef-backend-production.up.railway.app/produce/recognize-llm";
